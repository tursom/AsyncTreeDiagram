...
class AsyncSqliteHelper(base: String) : AsyncSqlHelper {
    ...
    override suspend fun insert(value: Any): Int {
        ...
        val valueStr = fields.valueStr(value, clazz) ?: return 0
        ...
        private val Field.fieldType: String?
            get() = when (type) {
                ...
                java.lang.String::class.java -> getAnnotation<TextLength>()?.let { "CHAR(${it.length})" } ?: "TEXT"
                    ...
                    val fieldType = getAnnotation(FieldType::class.java)
                    when {
                        fieldType != null -> fieldType.name
                        type.isSqlField -> type.getAnnotation<FieldType>()?.name ?: type.name.split('.').last()
                        else -> null
                    }
                }
            }
